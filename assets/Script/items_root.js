...
cc.Class({
    ...
    onConfigDataRead: function() {
        ...
        for(var i = this.maxRow - 1; i >= 0; --i) {
            ...
            for(var j = this.maxColumn - 1; j>= 0; --j) {
                ...
                itemInstant.setPosition(cc.p(j * this.itemWidth + this.itemWidth / 2,
                - this.itemHeight / 2));
                this.fallingItemArr[i][j] = itemInstant;

                itemInstant.runAction(cc.sequence(cc.delayTime(this.itemRiseTime * (2 - i)),
                     cc.moveBy(this.itemRiseTime * (i + 1),cc.p(0, (i + 1) * this.itemHeight)),
    ...
    getPosByColumn: function(column, row) {
        return cc.p(column * this.itemWidth + this.itemWidth / 2, row * this.itemHeight + this.itemHeight / 2);
    },
    ...
    checkDragItemFallPos: function(draggedItem) {
        ...
        draggedItem.setPositionX(this.getPosByColumn(column, fallRow).x);
        //下落动画
    ...
    checkNormalItemFallPos: function(normalItem) {
        ...
        var time = (pos.y - this.getPosByColumn(column, fallRow).y) / this.itemHeight * this.itemFallTime;
        normalItem.setPositionX(this.getPosByColumn(column, fallRow).x);
        //下落动画
    ...
    createNewLineInBottom: function(lineNum) {
        for(var i = 0; i < this.maxColumn; ++i){
            for (var line = 0; line < lineNum; ++line) {
                ...
                newItem.setPosition(cc.p(i * this.itemWidth + this.itemWidth / 2,
                - this.itemHeight / 2));
                this.setFallingItemByColumn(i, line, newItem);

                newItem.runAction(cc.sequence(cc.delayTime(this.itemRiseTime * (lineNum - line - 1)),
                    cc.moveBy(this.itemRiseTime * (line + 1),cc.p(0, this.itemHeight * (line + 1))),
        ...
        for(var row = 0; row < this.maxRow; ++row) {
            for(var column = 0; column < this.maxColumn; ++column) {
                ...
                if(item != null) {
                    ...
                    item.runAction(cc.sequence(cc.moveBy(this.itemRiseTime * lineNum,cc.p(0, this.itemHeight * lineNum)),
    ...
    createNewLineOnTop: function() {
        ...
        for(var i = 0; i < this.maxColumn; ++i) {
            ...
            newItem.setPosition(cc.p(i * this.itemWidth + this.itemWidth / 2, this.topLineCreateHeight));

            newItem.runAction(cc.moveBy(this.itemFallTime / 2,cc.p(0, -this.itemHeight / 2)));
    ...
    topNewLineShake: function() {
        for (var i = 0; i < this.topNewLineArr.length; ++i) {
            ...
            item.runAction(cc.repeatForever(cc.sequence(cc.moveBy(0.025, cc.p(-2, 0)),
                cc.moveBy(0.05, cc.p(4, 0)),
                cc.moveBy(0.025, cc.p(-2, 0)))));
        }
    },
    ...
    triggerPropItem: function(item) {
        ...
        switch(propType) {
            ...
            case EnumData.ItemTriggerStateEnum.Lightning: {
                ...
                columnSprite.setPositionY(this.getPosByColumn(column, row).y);
                rowSprite.setPositionX(this.getPosByColumn(column, row).x);
                    ...
                    this.node.runAction(cc.sequence(cc.delayTime(2.0 * index / elimateItemList.length),
                            cc.callFunc(function() {
                                ...
                                if (elimateItem.getComponent("item").getTriggerState() == EnumData.ItemTriggerStateEnum.Normal) {
                                    ...
                                    var distance = cc.pDistance(blackHolePos, elimateItem.getPosition());
                                    var radius = cc.pAngle(blackHolePos , elimateItem.getPosition()) * 180 / Math.PI - 60;
                                    var centerPos = cc.pAdd(cc.p(distance * Math.sin(radius), distance * Math.cos(radius)), elimateItem.getPosition());
    ...
    showCompositeItemsScore: function(itemsNum, itemType, pos) {
        ...
        scoreLabel.runAction(cc.sequence(cc.spawn(cc.moveBy(0.5, cc.p(0, this.itemHeight)).easing(cc.easeOut(3)), cc.fadeTo(0.5, 255)),
            cc.delayTime(0.3),
            cc.spawn(cc.moveBy(0.5, cc.p(0, this.itemHeight)), cc.fadeTo(0.5, 0)).easing(cc.easeIn(5)),
            cc.removeSelf()));
    ...
    showElimateItemScore(itemType, itemTriggerState, pos) {
        ...
        scoreLabel.runAction(cc.sequence(cc.spawn(cc.moveBy(0.3, cc.p(0, this.itemHeight)).easing(cc.easeOut(2)), cc.fadeTo(0.3, 255)),
            cc.delayTime(0.5),
            cc.spawn(cc.moveBy(0.3, cc.p(0, this.itemHeight)), cc.fadeTo(0.3, 0)).easing(cc.easeIn(2)),
            cc.removeSelf()));
